...
int comparator(const void *p1,const void *p2)//used by qsort()
{
	...
	return (*x)[2]-(*y)[2];// we are sorting the elements in ascending order so we have given it in this order
    // if we want to sort in descending order then we give (*y)[2]-(*x)[2]
}
